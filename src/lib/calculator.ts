...
function amountAndAccuracy(id: string, input: UserInput, standard: number): { estimatedAmount: string; accuracy: AccuracyLabel } {
  const monthly = estimatedMonthlyIncome(input);
  switch (id) {
    case "standard-monthly-remuneration": return { estimatedAmount: `${yen.format(standard)}（年収÷12=${yen.format(monthly)}から推定）`, accuracy: "中精度" };
    case "high-cost-medical-expense": return { estimatedAmount: highCostMedicalLimit(input, standard), accuracy: input.insuranceStatus === "employee" ? "中精度" : "要確認" };
    case "sickness-allowance": return { estimatedAmount: input.insuranceStatus === "employee" ? `${yen.format(dailyTwoThirdsAmount(standard, healthBenefits.sicknessAllowance))}/日（概算）` : "本人が健康保険の被保険者か要確認", accuracy: input.insuranceStatus === "employee" ? "中精度" : "要確認" };
    case "maternity-allowance": return { estimatedAmount: input.insuranceStatus === "employee" ? `${yen.format(dailyTwoThirdsAmount(standard, healthBenefits.maternityAllowance))}/日（概算）` : "被扶養者・国保は対象外または要確認", accuracy: input.insuranceStatus === "employee" ? "中精度" : "要確認" };
    case "child-allowance": return { estimatedAmount: childAllowanceEstimate(input), accuracy: "要確認" };
    case "childbirth-lump-sum": return { estimatedAmount: `${yen.format(healthBenefits.childbirthLumpSum.standardAmount)}（区分により${yen.format(healthBenefits.childbirthLumpSum.reducedAmount)}）`, accuracy: "高精度" };
    case "childcare-leave-benefit": return { estimatedAmount: `${yen.format(monthlyEmploymentBenefit(monthly, employmentBenefits.childcareLeaveBenefit))}/月（雇用保険加入なら目安）`, accuracy: "要確認" };
    case "family-care-leave-benefit": return { estimatedAmount: `${yen.format(monthlyEmploymentBenefit(monthly, employmentBenefits.familyCareLeaveBenefit))}/月（雇用保険加入なら目安）`, accuracy: "要確認" };
    case "burial-fee": return { estimatedAmount: `${yen.format(healthBenefits.burialFee.amount)}（国保は自治体の葬祭費を確認）`, accuracy: input.insuranceStatus === "national" ? "要確認" : "高精度" };
    case "survivor-disability-pension": return { estimatedAmount: pensionBenefits.survivorDisabilityPension.estimateLabel, accuracy: "要確認" };
    default: return { estimatedAmount: "要確認", accuracy: "要確認" };
  }
}
...
function detailFor(id: string, input: UserInput) {
  ...
  const map: Record<string, Omit<BenefitResult, keyof Benefit | "estimatedAmount" | "accuracy" | "note">> = {
